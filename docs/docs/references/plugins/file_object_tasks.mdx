...
A Nornir Result with ``changed`` indicating whether the object was created or updated, ``object_id`` with the UUID, and a descriptive ``result`` message.
...
A Nornir Result containing ``binary`` (base64 string), ``text`` (UTF-8 string or None), ``file_name``, ``file_type``, ``file_size``, ``checksum``, ``object_id``, ``save_to``, and ``changed`` (True only when a local file was created or overwritten).
