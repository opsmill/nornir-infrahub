...
- `name`: The Nornir property to set (e.g., "hostname", "platform")
- `mapping`: The path to the value in the Infrahub node (e.g., "primary_address.address")
...
When devices might have different attribute paths:

```yaml
# First try primary, fallback handled by multiple mappings
schema_mappings:
  - name: "hostname"
    mapping: "primary_address.address"
  - name: "hostname"
    mapping: "management_address.address"  # Used if first fails
```

### Custom Data Mappings

Include additional data for use in tasks:
...
def device_info(task):
    ...
    # Access custom data
    if "serial_number" in task.host.data:
        print(f"Serial: {task.host.data['serial_number']}")
