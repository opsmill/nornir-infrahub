...
Relationships traversed in `schema_mappings` or `group_mappings` are automatically added to `host_node.include`, so you don't need to list them manually. You can still specify `include` explicitly to limit which related fields are fetched (see [Include optimization](../topics/infrahub-inventory-concepts.mdx#include-optimization)).
