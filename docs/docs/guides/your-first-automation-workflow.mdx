...
snmp-server location {{ host.data.site }}
!
! NTP Configuration
...
def safe_deploy(task: Task) -> Result:
    try:
        ...
        if not task.host.is_reachable():
            return Result(
                host=task.host,
                failed=True,
                result="Host not reachable"
            )
...
def ci_workflow():
    ...
    if target_site:
        nr = nr.filter(site__name=target_site)
